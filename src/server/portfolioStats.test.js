...
import { dim, matrixFromArray, type Matrix } from "./linearAlgebra"

const priceMatrixMxN = la.transpose(matrixFromArray(2, testData.NYX.length, [testData.NYX, testData.INTC]))
...
describe("pStats", () => {
  ...
  describe("#mean()", () => {
    it("should calculate vector of mean values", () => {
      const expectedMean = matrixFromArray(3, 1, [[2.5], [25], [250]])
      const actualMean: Matrix<number, 3, 1> = pStats.mean(
        matrixFromArray(4, 3, [
          [1, 10, 100],
          [2, 20, 200],
          [3, 30, 300],
    ...
    it("should calculate vector mean values using test data", () => {
      var expectedMeanReturnRatesMatrix = matrixFromArray(2, 1, [[0.01674256058568205], [0.00504504938397936]])
      var returnRatesMatrix = pStats.calculateReturnRatesFromPriceMatrix(priceMatrixMxN, priceMatrixMxN.m - 1)
  ...
  describe("#covariance()", () => {
    it("[1] should calculate sample covariance", () => {
      ...
      const expected = matrixFromArray(3, 3, [
        [227.7, 285.75, 343.8],
        [285.75, 360.0, 434.25],
        [343.8, 434.25, 524.7]
      ])
      ...
      for (let i = 0; i < rowNum; i++) {
        for (let j = 0; j < colNum; j++) {
          actual.values[i][j] = utils.toFixedNumber(actual.values[i][j], 2)
        }
      }
    ...
    it("[2] should calculate sample covariance", () => {
      ...
      const expected = matrixFromArray(5, 5, [
    ...
    it("[3] should calculate sample covariance using testData", () => {
      // GIVEN
      var mXn = la.transpose(matrixFromArray(2, testData.NYX.length, [testData.INTC, testData.NYX]))
      var expected = matrixFromArray(2, 2, [
  ...
  describe("#calculateReturnRatesFromPriceMatrix()", () => {
    it("[1] should calculate return rates from price matrix", () => {
      // GIVEN
      const priceMatrix = matrixFromArray(3, 2, [
        [100.123, 1.123],
        [200.123, 2.123],
        [300.123, 3.123]
      ])
      ...
      // WHEN
      const actual = pStats.calculateReturnRatesFromPriceMatrix(priceMatrix, priceMatrix.m - 1)

      // THEN
      assertEqualMatrices(expected, actual, 5)
      assert.equal(actual.m, priceMatrix.m - 1)
    })
    it("[2] should throw up if not enough data points to calculate return rate", () => {
      // GIVEN
      const priceMatrix = matrixFromArray(1, 2, [[100.123, 1.123]]) // not enough data points to calculate return rates.
      var actual
      // WHEN
      try {
        pStats.calculateReturnRatesFromPriceMatrix(priceMatrix, priceMatrix.m - 1)
      } catch (e) {
        actual = e
      }
      // THEN
  ...
  describe("#portfolioStdDev()", () => {
    it("[1] should calculate portfolio Std Dev", () => {
      // GIVEN
      var weights1xN = matrixFromArray(1, 3, [[1 / 3, 1 / 3, 1 / 3]])
      var covarianceNxN = matrixFromArray(3, 3, [
        [0.01, 0.0018, 0.0011],
    ...
    it("[2] should calculate portfolio Std Dev", () => {
      // GIVEN
      const weights1xN = matrixFromArray(1, 3, [[0.2, 0.4, 0.4]])
      const covarianceNxN = matrixFromArray(3, 3, [
        [0.036224, 0.03298, 0.047716],
        [0.03298, 0.150345, 0.021842],
        [0.047716, 0.021842, 0.814886]
      ])
      const expected = 0.4193
      // WHEN
      const actual = pStats.portfolioStdDev(weights1xN, covarianceNxN)
      // THEN
